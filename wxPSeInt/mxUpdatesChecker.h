...
* "version" en la pagina (https://pseint.sourceforge.net/version) y lo compara su
...
*
* Para descargar el archivo, se utiliza curl o wget si está en el sistema.
**/
class mxUpdatesChecker : public wxDialog {
private:
	enum class Command { Autodetect, WGET, CURL, None };
	static Command m_command;
	static wxProcess *m_process;
	
	bool m_shown;
	wxStaticText *m_text;
	wxCheckBox *m_check;
//	wxTextCtrl *m_proxy_dir;
//	wxTextCtrl *m_proxy_port;
	wxButton *m_close_button;
	wxButton *m_changes_button;
//	wxButton *m_proxy_button;
public:
